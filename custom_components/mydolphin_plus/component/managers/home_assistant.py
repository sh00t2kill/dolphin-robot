...
class MyDolphinPlusHomeAssistantManager(HomeAssistantManager):
    ...
    def _load_sensor_cycle_time(
        ...
        try:
            ...
            cycle_time = datetime.timedelta(minutes=cycle_time_minutes)
    ...
    def _load_binary_sensor_broker_status(self, device_name: str):
        ...
        try:
            state = STATE_ON if self.ws.status == ConnectivityStatus.Connected else STATE_OFF

            attributes = {
                ATTR_FRIENDLY_NAME: entity_name,
                ATTR_STATUS: self.ws.status
            }
    ...
    def _load_sensor_cycle_time_left(
        ...
        try:
            ...
            state = datetime.timedelta(seconds=seconds_left).total_seconds()
    ...
    def _set_system_status_details(self, data: dict):
        ...
        if pws_error or robot_error:
            ...
            if (pws_cleaning and robot_cleaning) or (pws_programming and not robot_programming):
                calculated_state = ROBOT_STATE_INIT if robot_state == ROBOT_STATE_INIT else PWS_STATE_CLEANING
