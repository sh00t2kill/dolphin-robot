...
class Dolphin:
    ...
    def auth(self, username, password):
        ...
        response = requests.request(
            "POST", self.LOGIN_URL, headers=self.Headers, data=payload
        )
    ...
    def getToken(self):
        ...
        response = requests.request(
            "POST", self.TOKEN_URL, headers=self.Headers, data=payload
        )
        data = json.loads(response.text)
    ...
    def createAWSHeader(self, service, payload):
        ...
        signing_key = self.getSignatureKey(
            self.aws_secret, date_stamp, self.DYNAMODB_REGION, service
        )
        signature = hmac.new(
    ...
    def Query(self):
        payload = (
            '{"TableName":"maytronics_iot_history","Limit":1,"KeyConditionExpression":"musn = :val ","ScanIndexForward":false,"ExpressionAttributeValues":{":val":{"S":"'
            + self.serial
            + '"}}}'
        )
        service = "dynamodb"
    ...
    def buildClient(self):
        ...
        myAWSIoTMQTTClient = AWSIoTPyMQTT.AWSIoTMQTTClient(
            self.awsiot_id, useWebsocket=True
        )
        ...
        myAWSIoTMQTTClient.configureIAMCredentials(
            self.aws_key, self.aws_secret, self.aws_token
        )
